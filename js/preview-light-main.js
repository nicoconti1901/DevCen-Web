...
function initSparks() {
  ...
  function draw() {
    ...
    sparks.forEach((spark) => {
      ...
      if (spark.streak) {
        ctx.beginPath();
        ctx.strokeStyle = `rgba(217, 79, 0, ${0.28 + pulse * 0.42})`;
        ctx.lineWidth = 1.6;
        ctx.shadowColor = "#d94f00";
        ctx.shadowBlur = 16;
      ...
      ctx.shadowColor = spark.glow ? "#d94f00" : "rgba(217, 79, 0, 0.35)";
      ctx.shadowBlur = spark.glow ? 22 * pulse : 10;
    ...
    for (let i = 0; i < sparks.length; i += 1) {
      for (let j = i + 1; j < sparks.length; j += 1) {
        ...
        if (dist < 120 && (a.glow || b.glow)) {
          ctx.beginPath();
          ctx.strokeStyle = `rgba(217, 79, 0, ${0.22 * (1 - dist / 120)})`;
          ctx.lineWidth = 1.1;
          ctx.lineWidth = 1;
          ctx.moveTo(a.x, a.y);
          ctx.lineTo(b.x, b.y);
...
const protocol = [
  {
    title: "Diagnóstico",
    plain: "Primero escuchamos al equipo. Nos cuentan cómo trabajan hoy, con qué herramientas y dónde se traba el proceso. Sin este mapa no se promete un sistema.",
  ...
  {
    title: "Diseño",
    plain: "Acá se decide qué se construye y qué no. Ven el plano: módulos, quién entra, qué evidencia deja el sistema.",
    in: "El alcance que acabamos de acordar",
    out: "Esquema de módulos, permisos y datos",
    time: "Una iteración corta de diseño",
    close: "Validación conjunta del plano antes de programar",
  },
  {
    title: "Desarrollo",
    plain: "No desaparecemos tres meses. El cliente ve versiones, prueba y pide ajustes. El sistema se forma en ciclos cortos.",
...
const stickyPhrases = [
  "Demo sin cargo para validar la solución antes de construir.",
];
let stickyI = 0;
